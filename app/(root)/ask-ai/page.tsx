...
import { faArrowUp, faRobot, faStop, faUser } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="flex font-worksans text-white h-screen">
      ...
      <div className="flex-1 flex flex-col justify-between relative lg:px-10 max-sm:px-4 max-lg:mt-4">
        <div className="flex-1 overflow-y-auto px-10 mt-4 max-lg:mt-16 max-md:px-5 max-sm:px-1 pb-28">
          <div className="flex flex-col w-full text-left gap-4 whitespace-pre-wrap">
            {messages.map((m, index) => (
              <div className={`flex items-start gap-3 ${m.role === "user" ? "items-center" : "items-start"}`}>
                <div className="">
                {m.role !== "user" ? <FontAwesomeIcon icon={faRobot} className={`text-white mt-4`}/>:  <FontAwesomeIcon icon={faUser} className="text-white"/>}
                </div>
              <div
                key={index}
                className={`p-4 shadow-md rounded-xl relative ${
                  m.role === "user" ? "bg-slate-600 rounded-full" : "bg-slate-800"
                }`}
              >
                {m.content}
              </div>
        ...
        <div className="fixed bottom-0 left-0 lg:left-60 right-0 mx-auto max-w-6xl w-full px-4 bg-background py-4">
          ...
          >
            ...
            <button type="submit" className="bg-white rounded-full px-2 py-1">
              ...
              {isLoading ? <FontAwesomeIcon onClick={stop} icon={faStop} className="text-stone-600" width={16}/> : <FontAwesomeIcon
                icon={faArrowUp}
