...
import { createPrompt, updatePrompt } from "@/lib/actions/prompts.actions";
...
const UpdateForm = ({ userId, prompt, id, type }: PromptForm) => {
  const tagList = prompt.tags.map((tag) => tag.name);
  ...
  const initialValues = {
    ...
    collection: prompt.collection ? String(prompt.collection) : "",
  };
  ...
  async function onSubmit(values: z.infer<typeof promptFormSchema>) {
    ...
    const newForm = {
      ...values,
      tags,
      platform: selectedPlatforms ? selectedPlatforms : prompt.platform,
      collection: selectedCollection ? selectedCollection : prompt.collection,
    };
    // console.log(newForm);
