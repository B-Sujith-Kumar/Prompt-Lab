...
import EjectPrompt from "../Collections/EjectPrompt";

type CardProps = {
  prompt: IPrompt;
  collectionId: string;
};
const CollectionPromptCard = ({ prompt, collectionId }: CardProps) => {
  ...
  return (
    <div className="group relative font-worksans border-[0.7px] flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-background shadow-md transition-all hover:shadow-lg md:max-w-68 md:min-h-[350px] ">
      ...
      {isPromptCreator && (
        <Link
          href={`/prompt/update/${prompt._id}`}
          className="text-gray-500 shadow-lg border-black bg-white px-2 py-1 absolute top-4 right-3 rounded-lg"
        >
          <FontAwesomeIcon icon={faPenToSquare} />
        </Link>
      )}
      {isPromptCreator && <DeleteConfirmation promptId={prompt._id} />}
      {prompt.collection?.toString() !== collectionId && (
        <EjectPrompt promptId={prompt._id.toString()} collectionId={collectionId.toString()} />
      )}
      <Link
        href={`/prompt/${prompt._id}`}
